		...
		v1 := api.Group("/v1")
		{
			...
			protected.Use(middleware.AuthMiddleware(authService))
			{
				...
				admin.Use(middleware.RequireRole("admin"))
				{
					...
					admin.GET("/dlq", jobHandler.ListDLQ)
					admin.POST("/dlq/:job_id/retry", jobHandler.RetryFromDLQ)
					admin.POST("/dlq/:job_id/abandon", jobHandler.AbandonDLQ)
					admin.GET("/retry-config", jobHandler.GetRetryConfig)
					admin.PUT("/retry-config", jobHandler.UpdateRetryConfig)
