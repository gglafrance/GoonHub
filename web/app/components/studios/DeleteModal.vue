...
const { fetchAllStudioSceneIDs, deleteStudio } = useApiStudios();
...
const handleDelete = async () => {
    ...
    try {
        ...
        await deleteStudio(props.studio.id);
        emit('deleted');
    } catch (e: unknown) {
        error.value = e instanceof Error ? e.message : 'Failed to delete studio';
    } finally {
        loading.value = false;
    }
};
    ...
    <Teleport to="body">
        ...
        >
            <div class="glass-panel border-border w-full max-w-md border p-6">
                <h3 class="mb-2 text-sm font-semibold text-white">Delete Studio</h3>
                <div
                    v-if="error"
                ...
                <div class="bg-surface/50 border-border mb-4 rounded-lg border p-3">
                    ...
                    <div v-else class="text-xs">
                        <span class="text-dim">This studio has </span>
                ...
                <p v-if="!deleteScenes" class="text-dim mb-4 text-xs">
                    The studio will be removed from the system. Associated scenes will remain but
                    will no longer be linked to this studio.
                </p>
